...
def calculate_sprint_metrics(cards: List[Dict[str, Any]], lists: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Check for both "Done" and "Done 🎉" to handle both formats
        completed_cards = cards_by_list.get("Done 🎉", 0) + cards_by_list.get("Done", 0)
        ...
        # Identify critical tasks (tasks with red labels or high priority)
        critical_tasks = []
        for card in cards:
            is_critical = False
            ...
            # Check for high priority in name or description
            if not is_critical:
                name_lower = card.get("name", "").lower()
                desc_lower = card.get("desc", "").lower()
                if any(term in name_lower or term in desc_lower for term in ["high priority", "urgent", "critical"]):
                    is_critical = True
        ...
        metrics = {
            ...
            "completed_cards": completed_cards,  # Explicitly include completed cards count
            ...
            "critical_tasks": len(critical_tasks),  # Add critical tasks count
            "critical_tasks_list": critical_tasks  # Add list of critical tasks
        }
        ...
        return {
            "error": str(e),
            "total_cards": 0,
            "completed_cards": 0,  # Explicitly include completed cards count
            "cards_by_list": {},
            "completion_rate": 0,
            "blockers": [],
            "approaching_deadlines": [],
    ...
    try:
        ...
        for card in cards:
            ...
            for member_id in card_members:
                ...
                # Check if card is in the Done list (checking both formats)
                list_name = card.get("listName", "")
                if list_name in ["Done", "Done 🎉"]:
